...
\begin{enumerate}[i)]
    ...
    \item \textbf{Metal Weight-based Pricing}: Weight-based pricing calculation for jewellery items with configurable gold/silver/platinum rates. Real-time metal rate management with support for multiple purities (22K, 18K, 14K, 10K gold; 925, 999 silver; 900, 950 platinum).
...
\begin{enumerate}[i)]
    ...
    \item \textbf{Gold Rate \& Settings Management}: Centralized management of metal rates (gold, silver, platinum) by purity with historical tracking. Configure GST rates, shop details, tax configuration, and company information. API-ready configuration for automatic rate updates.
\end{enumerate}
...
\begin{enumerate}[i)]
    \item \textbf{Real-time Metal Price Fetching}: System architecture designed for real-time metal price updates from market APIs. Currently supports manual updates via admin dashboard with instant propagation to invoice system. Infrastructure ready for integration with external metal pricing APIs for automated rate fetching (gold, silver, platinum rates with purity factors).
...
\textbf{Real-time Metal Price Fetching and API Integration}:
\begin{enumerate}[i)]
    \item \textbf{Multi-Metal Support}: Real-time fetching and management of gold, silver, and platinum rates with multiple purity standards
    \item \textbf{Purity-Based Pricing}: Automatic calculation of metal prices based on weight and purity coefficients (22K, 18K, 14K gold; 925, 999 silver; 900, 950 platinum)
    \item \textbf{Rate History Tracking}: Comprehensive logging of all metal rate changes with timestamps for audit trail and price analysis
    \item \textbf{Manual Rate Updates}: Real-time rate updates via admin dashboard with immediate application to new invoices
    \item \textbf{API Rate Endpoints}: RESTful API endpoints for programmatic access to current rates and historical data (GET /api/products/gold-rate)
    \item \textbf{External API Integration Ready}: Architecture supports integration with external market data APIs for automated rate fetching (configuration fields available for API keys)
    \item \textbf{Instant Rate Propagation}: Updated metal rates instantly available to invoice creation system without cache delays
    \item \textbf{Rate Snapshot Mechanism}: Metal rates locked at invoice creation time ensuring consistent pricing regardless of subsequent rate changes
    \item \textbf{Multiple Metal Types}: Simultaneous management of gold, silver, and platinum rates each with configurable purity-based pricing
    \item \textbf{Calculate Price in Real-time}: Formula: Metal Price = Weight (grams) × Current Metal Rate (₹/gm) × Purity Factor
\end{enumerate}

\textbf{Implementation Highlights}:
\begin{lstlisting}[language=JavaScript, caption=Gold Rate Settings API with Real-time Fetching]
// GET current settings and rates (real-time)
GET /api/settings
Response: {
    metalRates: {
        gold: { purity22K: 7500, purity18K: 5625, purity14K: 4200, purity10K: 3000 },
...
// GET gold rate with real-time purity breakdown
...
// PUT - Update rates in real-time (reflects immediately in system)
PUT /api/products/gold-rate {
    metalRates: {
        gold: { purity22K: 7550, purity18K: 5662.5, ... },
        silver: { purity999: 76, purity925: 70.2 },
        platinum: { purity950: 1450, purity900: 1375 }
    }
}

// Calculate invoice price using real-time rates:
// itemPrice = weight × (metalRate × purityFactor) + laborCharge + (amount × GST%)
// Gold 22K, 10gm @ ₹7500/gm, Purity 0.916: 10 × 7500 × 0.916 = ₹68,700
\end{lstlisting}
